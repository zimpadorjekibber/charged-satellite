    ...
    return (
        <div className="pb-24 pointer-events-auto">
            ...
            {valleyUpdates.length > 0 && (
                <div className="mt-12 bg-neutral-900/50 rounded-2xl p-6 border border-white/5 mx-1">
                    <h3 className="text-tashi-accent font-serif text-xl font-bold mb-4 flex items-center gap-2">
                        <Newspaper size={20} /> Valley Updates
                    </h3>
                    <div className="space-y-4">
                        {(showAllUpdates ? valleyUpdates : valleyUpdates.slice(0, 2)).map((update, idx) => (
                            <div key={idx} className={`bg-black/40 p-4 rounded-xl border-l-2 ${update.statusColor === 'green' ? 'border-green-500' :
                                update.statusColor === 'blue' ? 'border-blue-500' :
                                    update.statusColor === 'red' ? 'border-red-500' : 'border-gray-500'
                                }`}>
                    ...
                    {valleyUpdates.length > 2 && (
                        <button
                            onClick={() => setShowAllUpdates(!showAllUpdates)}
                            className="w-full mt-4 text-xs font-bold text-gray-400 hover:text-white py-2 bg-white/5 rounded-lg transition-colors border border-white/5"
                        >
                            {showAllUpdates ? 'Show Less' : `View All Updates (${valleyUpdates.length})`}
                        </button>
                    )}
