...
export default function MenuPage() {
    ...
    // Track previous pending state to detect "Resolved" transition
    const prevPendingRef = useRef(hasPendingCall);

    // Audio State Management (Reactive)
    useEffect(() => {
        const audio = callSoundRef.current;
        if (isCalling && audio) {
            audio.play().catch(e => console.error("Audio play failed", e));
        } else if (!isCalling && audio) {
            audio.pause();
            audio.currentTime = 0;
        }
    }, [isCalling]);

    // Sync Store State -> UI State
    useEffect(() => {
        // 1. If Server says pending, ensure UI matches (e.g. page refresh)
        if (hasPendingCall) {
            setIsCalling(true);
        }

        // 2. If Server transitions True -> False (Resolved by staff), stop UI
        if (prevPendingRef.current && !hasPendingCall) {
            setIsCalling(false);
        }

        prevPendingRef.current = hasPendingCall;
    }, [hasPendingCall]);
    ...
    const handleCallStaff = async () => {
        ...
        if (isCalling) {
            if (isCalling) {
                // Stop local visual feedback (Audio handled by useEffect)
                setIsCalling(false);
            ...
            try {
                // Optimistic Update: Assume success immediately (Audio handled by useEffect)
                setIsCalling(true);
